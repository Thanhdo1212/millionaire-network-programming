...
// get money
int score[15] = {200000, 400000, 600000, 1000000, 2000000, 3000000, 6000000, 10000000, 14000000, 22000000, 30000000, 40000000, 80000000, 150000000, 250000000};
struct user {
...
void init() {
    ...
    fprintf(f, "thanh thanh123 1\n");
    fprintf(f, "quang quang123 1\n");
    fprintf(f, "tuan tuan123 1\n");
    fclose(f);
}
	...
	while((n = recv(connfd, buff, BUFF_SIZE, 0)) > 0) {
        ...
        if (strcmp(buff, "1") == 0) {
            if (cli->login_status == 0) {
                ...
                int sn = snprintf(mesg, sizeof(mesg), "Bạn đang đăng nhập bằng tài khoản %s.", cli->login_account);
                if (send(connfd, mesg, strlen(mesg), 0) < 0){
                    perror("Send error");
                    deleteClient(connfd);
                    return NULL;
        ...
        // Register
            ...
            if (cli->login_status == 0) {
                ...
                int sn = snprintf(mesg, sizeof(mesg), "Bạn đang đăng nhập bằng tài khoản %s. Hãy đăng xuất trước.", cli->login_account);
                if (send(connfd, mesg, strlen(mesg), 0) < 0){
                    perror("Send error");
                    deleteClient(connfd);
                    return NULL;
        ...
        // Play online
            ...
            if (cli->login_status == 0) {
                ...
                if (room_entered == -1) {
                    int sn = snprintf(mesg, sizeof(mesg), "Player [%d - %s] enter room failed!", cli->connfd, cli->login_account);
                    printf(CYN "%s\n" RESET, mesg);
                    ...
                    int sn = snprintf(mesg, sizeof(mesg), "Player [%d - %s] enter room %d.", cli->connfd, cli->login_account, room_entered);
                    printf(CYN "%s\n" RESET, mesg);
                    sprintf(mesg, "[Phòng %d] Ghép cặp thành công. Bắt đầu!\n", room_entered);
                    if (send(connfd, mesg, strlen(mesg), 0) < 0) {
                        int sn = snprintf(mesg, sizeof(mesg), "Người chơi [%d - %s] đã thoát.", cli->connfd, cli->login_account);
                ...
                if (connfd == Game.room[room_entered][0]) {
                    ...
                    if (step == SQLITE_ROW) { 
                        ...
                        int sn = snprintf(mesg, sizeof(mesg), "Câu hỏi phụ: %s\nA. %s\nB. %s\nC. %s\nD. %s\nĐáp án của bạn:\n",
                        sqlite3_column_text(res, 0),
                        sqlite3_column_text(res, 1),
                        sqlite3_column_text(res, 2),
                        sqlite3_column_text(res, 3),
                        ...
                        //send_to_others(mesg, connfd, room_entered);
                        printf("%s (%lu bytes)\n", mesg, strlen(mesg));
                        Game.true_answer[room_entered] = atoi((char *)sqlite3_column_text(res, 5));
                    }
                }
                // code gpt 
//                 sqlite3 *db;
...
// end gpt
                ...
                buff[n] = '\0';
                int sn = snprintf(mesg, sizeof(mesg),"[%d - %s]: %s\n", cli->connfd, cli->login_account, buff);
                    ...
                    while (Game.room_status[room_entered] != 0) {
                        if ((n = recv(connfd, buff, strlen(buff), MSG_DONTWAIT)) <= 0) {
                            if (errno != EAGAIN && errno != EWOULDBLOCK) {
                                int sn = snprintf(mesg, sizeof(mesg), "Người xem [%d - %s] đã thoát.\n", cli->connfd, cli->login_account);
                                send_to_others(mesg, connfd, room_entered);
                                resetGameRoom(room_entered, connfd);
                                printf("Host disconnected: socket_fd(%d)\n", connfd);
                                deleteClient(connfd);
                ...
                // Main player only
                    ...
                    int sn = snprintf(mesg, sizeof(mesg), "Người chơi: " CYN "%s" RESET "\nKhán giả:", tmp->login_account);
                    for (int j = 0; j < Game.room_size[room_entered]; j++) {
                        ...
                        if (Game.room[room_entered][j] != Game.main_player[room_entered]) {
                            ...
                            int sn = snprintf(str, sizeof(str), CYN " %s" RESET, tmp->login_account);
                            strcat(mesg, str);
                        }
                    }
                    strcat(mesg, "\n");
                    ...
                    for (i = 0; i < 15; i++) {
                        if (help == 0) {
                            ...
                            if (step == SQLITE_ROW) { 
                                ...
                                printf("Đáp án câu %d: %s\n", i + 1, sqlite3_column_text(res, 5));
                                ...
                                if (i > 0) {
                                    ...
                                    char notifyGetMoney[100]; // Adjust the size as needed
                                    notifyGetMoney[0] = '\0'; // Initialize the string

                                    if (i == 5 || i == 10) {
                                        getMoneyDefault = score[i-1];
                                        snprintf(notifyGetMoney, sizeof(notifyGetMoney), "\nBạn đã đạt được mốc giải thưởng %d vnđ", score[i-1]);
                                    }
                                    int sn = snprintf(mesg, sizeof(mesg), "Chính xác!!%s\nBạn đang ở mốc giải thưởng %d vnđ\nbạn đang có mức tiền thưởng là: %d Vnđ \n\n %s", notifyGetMoney, getMoneyDefault, score[i-1], tmp);
                                    notifyGetMoney[0] = '\0'; // Clear the string for future use
                                }
                        ...
                        help = 0;
                        if ((n = recv(connfd, buff, BUFF_SIZE, 0)) <= 0) {
                            int sn = snprintf(mesg, sizeof(mesg), "Người chơi chính [%d - %s] đã thoát.\n Mode online kết thúc! \n Người chơi dành được giải thưởng %d vnđ", cli->connfd, cli->login_account, score[i-1]);
                            send_to_others(mesg, connfd, room_entered);
                            printf("[Room %d]: %s\n", room_entered, mesg);
                        ...
                        if(atoi(answer) == 7){
                            ...
                            help = 1; 
                            i--;
                            //printf("Đang ở câu số(trước if else): %d\n", i);
                            if (j == 0) {
                                ...
                                j--;
                                do k = (rand() % (3 - 1 + 1)) + 1; // (upper - lower + 1)) + lower
                                while (k == atoi((char *)sqlite3_column_text(res, 5)));
                                
                                char option1[BUFF_SIZE], option2[BUFF_SIZE];

                                if (k == 1) strcpy(option1, (char *)sqlite3_column_text(res, 1));
                                else if (k == 2) strcpy(option1, (char *)sqlite3_column_text(res, 2));
                                else if (k == 3) strcpy(option1, (char *)sqlite3_column_text(res, 3));
                                else strcpy(option1, (char *)sqlite3_column_text(res, 4));

                                printf("%s\n", (char *)sqlite3_column_text(res, 5));
                                int correct = atoi((char *)sqlite3_column_text(res, 5));
                                if (correct == 1) strcpy(option2, (char *)sqlite3_column_text(res, 1));
                                else if (correct == 2) strcpy(option2, (char *)sqlite3_column_text(res, 2));
                                else if (correct == 3) strcpy(option2, (char *)sqlite3_column_text(res, 3));
                                else strcpy(option2, (char *)sqlite3_column_text(res, 4));
                                int sn;
                                if (k < correct)
                                    sn = snprintf(mesg, sizeof(mesg), "Bạn đã sử dụng sự trợ giúp 50/50 " CYN "(Còn %d lượt)" RESET "\nCâu hỏi %d: %s\n%c. %s\n%c. %s\nĐáp án của bạn: ",
                                            j, i+2, sqlite3_column_text(res, 0), 
                                            to_ABCD(k), option1, to_ABCD(correct), option2);
                                else
                                    sn = snprintf(mesg, sizeof(mesg), "Bạn đã sử dụng sự trợ giúp 50/50 " CYN "(Còn %d lượt)" RESET "\nCâu hỏi %d: %s\n%c. %s\n%c. %s\nĐáp án của bạn: ",
                                            j, i+2, sqlite3_column_text(res, 1), 
                                            to_ABCD(correct), option2, to_ABCD(k), option1);
                                printf("%s\n", mesg);
                                send_to_others(mesg, -1, room_entered);
                            }
                            // pthread_mutex_unlock(&mutex);
                        }
                        else if (strcmp(answer, (char *)sqlite3_column_text(res, 5)) != 0) {
                            if (i == 0) sprintf(mesg, "Sai! Đáp án đúng là %c\nKém quá, bạn chưa trả lời đúng câu nào 🤣 🤣\n", to_ABCD(atoi((char *)sqlite3_column_text(res, 5))));
                            else {
                                int getMoney;
                                if(i > 0 && i < 5) getMoney = 0;
                                else if (i >= 5 && i < 10) getMoney = score[4];
                                else if (i >= 10 && i < 15)getMoney = score[9];
                                sprintf(mesg, "Sai! Đáp án đúng là %c\nBạn đã trả lời đúng %d câu hỏi.\n giải thưởng bạn nhận được là: %d vnđ \n", to_ABCD(atoi((char *)sqlite3_column_text(res, 5))), i, getMoney);
                            }
                            printf("%s\n", mesg);
                            send_to_others(mesg, -1, room_entered);
                            break;
                    ...
                    if (i == 15) {
                        ...
                        snprintf(mesg, sizeof(mesg), "\nChúc mừng bạn đã trả lời đúng 15 câu hỏi! và nhận được giải thưởng trị giá %d đồng ️🎉️ 🎉\n\n", score[i-1]);
                        printf("%s\n", mesg);
                        send_to_others(mesg, -1, room_entered);
                    }
                    sqlite3_finalize(res);
            ...
            if (cli->login_status == 0) {
                ...
                for (i = 0; i < 15; i++) {
                    ...
                    if (help == 0) { 
                        ...
                        else if (i != 0) sn = snprintf(mesg, sizeof(mesg), "Chính xác!\n\n%s", tmp);
                        else strcpy(mesg, tmp);
                        printf("%s\n", mesg);
                        if (send(connfd, mesg, strlen(mesg), 0) < 0) {
                            perror("Send error");
...
void send_to_others(char *mesg, int current_player, int room_number){
    ...
    for (int i = 0; i < Game.room_size[room_number]; i++) {
        ...
        printf("Send_to: %d (%lu bytes)\n", Game.room[room_number][i], strlen(mesg));
    }
	pthread_mutex_unlock(&mutex);
}
